...
class UltraApiClient {
  ...
  async getOrder(params: OrderParams): Promise<OrderResponse> {
    try {
      ...
      url.searchParams.append("referralAccount", params.referralAccount);
      url.searchParams.append("referralFee", params.referralFee.toString());
...
async function orderAndExecute() {
  ...
  const orderParams: OrderParams = {
    ...
    referralAccount: "YOUR_REFERRAL_ACCOUNT", // This will be the referral account that receives the referral fee (not the referral token account)
    referralFee: 50, // This will be the referral fee in bps
  };
